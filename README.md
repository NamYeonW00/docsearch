...
<img width="656" height="571" alt="image" src="https://github.com/user-attachments/assets/a1527d92-74b1-43be-8168-c478a04cc112" />
...
| Chunking | Spring AI `TokenTextSplitter` | chunkSize 등 설정 (overlap은 Spring AI가 미지원) |
...
GET    /api/documents?title=                    제목 부분 일치 검색 (대소문자 무시, 활성 문서 목록)
...
│     ├─ common/
│     │  └─ GlobalExceptionHandler.java   -- 전역 예외 매핑(@RestControllerAdvice): 404/400/413/503
│     ├─ document/
...
│     │  ├─ AiServiceException.java    -- LLM 호출 실패/타임아웃 → 503
│     │  └─ dto/                       -- ChatResponseDto
│     ├─ chunking/
│     │  ├─ DocumentChunker.java       -- TokenTextSplitter 래핑
│     │  └─ ChunkingProperties.java    -- chunkSize/minChunkSizeChars/minChunkLengthToEmbed/maxNumChunks
│     └─ pdf/
│        ├─ PdfTextExtractor.java       -- 업로드 PDF 바이트 → 텍스트 추출 (PagePdfDocumentReader)
│        └─ PdfProcessingException.java -- PDF 입력 문제(비-PDF/빈 파일/텍스트 없음/손상) → 400
│
...
8. **프론트엔드 화면**: 문서 등록 폼, 조회 화면, 검색 화면, 채팅 화면 구현 ✅
...
- [x] 버전 이력 조회 API (`GET /documents/title/{title}/versions`)
- [x] 문서 삭제 (soft delete)
- [x] Metadata Filtering (category별 검색)
- [x] Score Threshold 적용
- [x] PDF 파일 업로드 후 자동 임베딩
